...
let secureAuth = async (req: Request, res: Response) => {
  try {
    ...
    // Primero intentamos buscar el usuario en ambas tablas usando los nombres correctos de columnas
    ...
    if (adminResult.length > 0) {
      ...
      if (isPasswordValid) {
        return res.status(200).json({
          ...
          userType: "admin",
          token: generateToken({id: adminResult[0].id_admin, role: "admin"}, process.env.KEY_TOKEN, 60)
        });
      }
    }
